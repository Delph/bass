...
function isPersistedBucket(value: unknown): value is PersistedBucket<unknown> {
  return typeof value === 'object' && value !== null && 'version' in value && 'data' in value && typeof value.version === 'number';
}
...
export function defineBucket<T>(bucket: Bucket<T>) {
  ...
  function save(data: T) {
    return localStorage.setItem(bucket.key, JSON.stringify({version: bucket.version, data} satisfies PersistedBucket<T>));
  }
